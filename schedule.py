...
class Teacher():
    ...
    def __init__(self, name, teacher_type='vnfulltime', 
            main_classes=None,
            improper_classes=None,
            predefined_days_off=None,
            number_of_off_days=1,asup=False):
...
class Schedule():
    ...
    def get_main_teacher_name(self, cl):
        result = None
        for teacher in self.teachers.values():
    ...
    def get_least_busy_teacher(self, teachers, teacher_type):
        min_number = 999
        selected = None
        for teacher in teachers:
            # prevent the case when parttime teaches when asup is still free
            if teacher_type== 'vnparttime' and teacher.asup==True:
                return teacher

            if teacher.type == teacher_type or teacher.asup==True:
                # asup will be the last one doing backup
                if teacher.asup==True:
                    if len(teachers)==1:
                        return teacher
                    else:
                        continue

                if teacher.number_of_lessons<min_number:
                    min_number = teacher.number_of_lessons
                    selected = teacher
        if selected!=None:
            return selected
        else:
            ...
            for shift in shifts:
                ...
                for cl in remaining_classes:
                    if cl not in temp_classes_for_part_time_teachers:
                        proper_teachers = []
                        for teacher in remaining_teachers:
                            if cl not in teacher.improper_classes and teacher not in temp_parttime_teachers_allocation:
                        ...
                        if least_busy_teacher!=None:
                            temp = 'arrange remaining full time teachers to remaining classes which dont have main teacher ' + cl.class_code + ' - ' +least_busy_teacher.name + ' - native: ' + str(cl.get_native_need_rate()) + ' - main vn: ' + str(cl.get_main_vn_teacher_need_rate())
                            class_teacher_allocations[shift].append(temp)
                            cl.increase_learned_lesson_number()
                            teacher.number_of_lessons += 1
                ...
                for cl in remaining_classes:
                    ...
                    if cl not in temp_classes_for_part_time_teachers:
                        ...
                        if least_busy_teacher!=None:
                            ...
                            temp = 'arrange remaining part time teachers remaining classes which dont have main teacher ' + cl.class_code + ' - ' +least_busy_teacher.name + ' - native: ' + str(cl.get_native_need_rate()) + ' - main vn: ' + str(cl.get_main_vn_teacher_need_rate())
                            class_teacher_allocations[shift].append(temp)
                            cl.increase_learned_lesson_number()
                            teacher.number_of_lessons += 1
                ...
                for cl in temp_classes_for_part_time_teachers:
                    proper_teachers = []
                    for teacher in remaining_teachers:
                        if cl not in teacher.improper_classes:
                            if not any(teacher.name in string for string in class_teacher_allocations[shift]):
                                proper_teachers.append(teacher)
                    
                    least_busy_teacher = self.get_least_busy_teacher(proper_teachers, 'vnfulltime')
                    if least_busy_teacher!=None:
                        temp = 'arrange remaining full time teachers to partime classes which have main teacher ' + cl.class_code + ' - ' +least_busy_teacher.name + ' - native: ' + str(cl.get_native_need_rate()) + ' - main vn: ' + str(cl.get_main_vn_teacher_need_rate())
                        class_teacher_allocations[shift].append(temp)
                        cl.increase_learned_lesson_number()
                        teacher.number_of_lessons += 1
    ...
    def calculate_assessments(self, schedule):
        ...
        for each_string in fulltiem_teachers_classes_list:
            ...
            if teachers[each_string].asup==False:
                lesson_counts.append(self.teachers[each_string].lesson_count )
...
# todo: 
    ...
    def find_best_schedule(self):
        ...
        for off_combination in off_combinations:
            s = time.time()
            print((['Ms.Amy-tue'], ['Ms.Mia-mon'], ['Mr.Kay-wed'], ['Mr.Andre-wed', 'Mr.Andre-tue'], ['Ms.Jane-mon'], ['Ms.Jay-mon'], ['Mr.Matt-fri', 'Mr.Matt-thu']))
            result_schedule = self.put_schedule((['Ms.Amy-tue'], ['Ms.Mia-mon'], ['Mr.Kay-wed'], ['Mr.Andre-wed', 'Mr.Andre-tue'], ['Ms.Jane-mon'], ['Ms.Jay-mon'], ['Mr.Matt-fri', 'Mr.Matt-thu']))
...
def test():
    ...
    teachers['Mr.Kay'].predefined_days_off = ['sun']
    ...
    teachers['Mr.Kay'].improper_classes.append(classes['SK1B1-121220'])
